#!/bin/sh
#
# This script is for RedHat variants only. 
...
#
    ...
    if [ "`grep '6\.' /etc/redhat-release`" ];then
        sudo echo "#!/bin/sh" > /etc/dhcp/dhclient.d/ifup-eth0:1.sh 
        sudo echo "/sbin/ifup eth0:1 &> /dev/null" >> /etc/dhcp/dhclient.d/ifup-eth0:1.sh 
        sudo chmod +x /etc/dhcp/dhclient.d/ifup-eth0:1.sh
    else
        echo -e "\nThis is not necessary for Fedora 5, or CentOS 5.2 & 5.6 systems\n"
    fi

    # Bring up eth0:1 while we're at it.
