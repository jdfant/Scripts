...
# Download, install, and configure OSSEC and SSHguard.
# Create OSSEC Agent keys on Server and post keys to import to OSSEC Agent. 
#

# This should not change, but just in case ....
...
apt_setup(){
    if [ -e /etc/debian_version ];then
        ...
        apt-get -qq update
        apt-get -qqy install ossec-hids-agent sshguard
    else
...
sshguard_conf(){
    echo -e "${ossec_server}\nxxx.xxx.xxx.xxx # Office" >> /etc/sshguard/whitelist
}
